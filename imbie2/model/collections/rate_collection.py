...
class MassRateCollection(Collection):
    def average(self, mode=None):
        raise NotImplementedError(
            "method not available for raw dm/dt data (chunk this collection first)"
        )
...
class WorkingMassRateCollection(Collection):
    ...
    def average(
        self,
        mode: AverageMethod = AverageMethod.equal_groups,
        nsigma: float = None,
        error_mode: ErrorMethod = ErrorMethod.rss,
        ...
        if error_mode in [
            ...
            for i, s in enumerate(self):
                i_m, i_s = match(t, s.t, 1.0 / 48)
                devs[i, i_m] = s.dmdt[i_s] - m[i_m]

            e = np.sqrt(np.nanmean(np.square(devs), axis=0))

            all_members = np.isfinite(
                np.product(devs, axis=0)
            )  # find epochs w/o nan in 'devs'
            ...
            if error_mode == ErrorMethod.constant_dev:
                ...
                _, rms_e = ts_combine(
                    ts, es, nsigma=nsigma, error_method=ErrorMethod.rms
                )
                sqr_error = np.vstack([rms_e, const_e]) ** 2.0
    ...
    def monthly(self) -> "WorkingMassRateCollection":
        out = WorkingMassRateCollection()
        for series in self:
            out.add_series(series.monthly())
        return out
    ...
    def sum(self, error_method=ErrorMethod.sum) -> WorkingMassRateDataSeries:
        ...
        if any(s.trunc_extent is None for s in self):
            ...
            avg_min_margin = np.mean([abs(s.trunc_extent[0] - s.t.min()) for s in self])
            avg_max_margin = np.mean([abs(s.trunc_extent[1] - s.t.max()) for s in self])
    ...
    def integrate(
        self, offset=None, align=None
    ) -> "model.collections.MassChangeCollection":
        out = model.collections.MassChangeCollection()
        for series in self:
    ...
    def smooth(
        self, window=None, clip=False, taper=False, iters=1
    ) -> "WorkingMassRateCollection":
        if window is None:
            return self
        out = WorkingMassRateCollection()
        for s in self:
            out.add_series(s.smooth(window=window, clip=clip, taper=taper, iters=iters))
        return out
    ...
    def reduce(self, interval: float = 1.0, centre=None, backfill=False):
        out = WorkingMassRateCollection()
        for s in self:
            out.add_series(
                s.reduce(interval=interval, centre=centre, backfill=backfill)
            )
        return out

    def __add__(
        self, other: "WorkingMassRateCollection"
    ) -> "WorkingMassRateCollection":
    ...
    def get_window(
        ...
        return WorkingMassRateCollection(
            *[
                s.truncate(min_time, max_time, interp=interp)
                for s in self
                if np.any(s.t < max_time) and np.any(s.t > min_time)
            ]
        )
    ...
    def stdev(self) -> float:
        return np.nanstd(np.hstack([s.dmdt for s in self.series]))

    def merge_basin_types(self) -> "WorkingMassRateCollection":
        """
        produce average of rignot & zwally basins where users have
        provided definitions for the same basins for both
        """
        users = {s.user for s in self}
        out = WorkingMassRateCollection()

        for user in users:
            user_collection = self.filter(user=user)

            basins = {s.basin_id for s in user_collection}

            for basin in basins:

                basin_collection = user_collection.filter(basin_id=basin)

                if len(basin_collection) == 1:
                    out += basin_collection
                    continue

                basin_types = {s.basin_group for s in basin_collection}

                assert len(basin_types) == len(
                    basin_collection
                ), f"unresolvable basin configuration: {user}/{basin.value}"

                avg_series = basin_collection.average()
                avg_series.contributions = 1
                avg_series.basin_group = BasinGroup.mean
                avg_series.user = user
                print(f"merged rignot/zwally for: {avg_series.user}/{basin.value}")

                out.add_series(avg_series)
